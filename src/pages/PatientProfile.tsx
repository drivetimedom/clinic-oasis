...
export default function PatientProfile() {
  ...
  const { data: patient } = useQuery({
    queryKey: ["patient", id],
    queryFn: async () => {
      const { data } = await supabase.from("patients").select("*").eq("id", id!).eq("user_id", user!.id).single();
      return data;
    },
    enabled: !!user && !!id,
  });
  ...
  const { data: receivables = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("receivables").select("*").eq("patient_id", id!).order("due_date", { ascending: false });
      return data || [];
    },
    enabled: !!id,
  });
      ...
      <Card>
        ...
        <CardContent>
          {receivables.length === 0 ? (
            <p className="text-muted-foreground text-sm text-center py-4">Nenhum registro financeiro.</p>
          ) : (
              ...
              {receivables.map((r) => (
                <div key={r.id} className="flex items-center justify-between p-3 rounded-lg bg-secondary/50">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Venc: {formatDate(r.due_date)}</p>
                  </div>
                  <div className="flex items-center gap-3">
                    <Badge className={getStatusColor(r.status)}>{getStatusLabel(r.status)}</Badge>
                    <span className="font-bold">{formatCurrency(Number(r.amount))}</span>
                  </div>
                </div>
              ))}
            </div>
